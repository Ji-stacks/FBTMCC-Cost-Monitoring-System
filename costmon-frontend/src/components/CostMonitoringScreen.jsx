      ...
      if (d.costing_type === 'normal' && d.expenses && d.expenses.length > 0) {
        d.expenses.forEach(exp => {
          ...
          grouped[targetCat].push({
            id: d.id, lineId: exp.id, date: d.date, cv_no: d.cv_no, or_inv_no: d.or_inv_no, po_no: d.po_no,
                      ...
                      <table className="w-full text-left border-collapse text-xs" style={{ zoom: zoomLevel }}>
                        <thead>
                          {isMisc ? (
                            ...
                            <>
                              <tr className={`${headerColor} border-b-2 border-slate-800 dark:border-slate-600`}><th colSpan={canEdit ? 16 : 15} className="text-center py-3.5 font-black text-white uppercase tracking-[0.15em] text-sm">{category}</th></tr>
                        ...
                        <tbody className="divide-y divide-slate-800 dark:divide-slate-600 bg-white dark:bg-slate-800">
                          {items.map((item, i) => (
                            <tr key={`${item.id}-${i}`} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-[11px]">
                              ...
                              <td className="p-3 text-center font-mono font-bold text-slate-700 dark:text-slate-300 border-r border-slate-800">{item.po_no || '-'}</td>
                          ...
                          <tr className="bg-slate-100 dark:bg-slate-700/80 border-t-2 border-slate-800 dark:border-slate-600">
                            <td colSpan={isMisc ? 13 : 12} className="p-3 text-right font-black text-[11px] uppercase tracking-widest text-slate-800 dark:text-slate-200 border-r border-slate-800">TOTAL FOR {category}:</td>
