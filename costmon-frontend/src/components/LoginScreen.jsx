...
export default function LoginScreen({ onLogin }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      {/* Background decoration kapag nasa forms na */}
      {view !== 'role-select' && (
        <div className="absolute top-0 left-0 w-full h-96 bg-indigo-600 rounded-b-[100px] shadow-lg -z-10 transform -translate-y-20 transition-all duration-500"></div>
      )}

      {/* BRANDING SECTION - Laging nakikita */}
      <div className={`flex flex-col items-center text-center transition-all duration-500 ${view === 'role-select' ? 'mb-12' : 'mb-8'}`}>
        <div className={`p-4 rounded-2xl mb-4 transform transition-all duration-500 ${view === 'role-select' ? 'bg-indigo-600 text-white shadow-2xl shadow-indigo-200 rotate-3 hover:rotate-0 p-6 rounded-[2.5rem]' : 'bg-indigo-100 text-indigo-600'}`}>
          <LayoutDashboard size={view === 'role-select' ? 64 : 40} strokeWidth={2.5} />
        </div>
        <h1 className={`font-black text-slate-800 tracking-tight leading-none uppercase ${view === 'role-select' ? 'text-5xl' : 'text-3xl'}`}>
          FBTMCC <span className={`text-indigo-600 block mt-1 ${view === 'role-select' ? 'text-4xl mt-2' : 'text-xl'}`}>COST MONITORING</span>
        </h1>
        {view === 'role-select' && (
          <>
      ...
      {/* VIEW: ROLE SELECTION */}
      {view === 'role-select' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl px-4 animate-in fade-in zoom-in-95 duration-300">
      ...
      {/* VIEW: FORMS (Login, Forgot Username, Forgot Password) */}
      {view !== 'role-select' && (
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 border border-slate-100 animate-in slide-in-from-bottom-4">
          ...
          {view === 'login' && (
            <form onSubmit={handleLoginSubmit} className="space-y-5">
              <div className="text-center mb-6 border-b border-slate-100 pb-4">
                ...
                <h2 className="text-xl font-black text-indigo-600 uppercase mt-1">{selectedRole === 'ceo' ? 'PRESIDENT' : selectedRole}</h2>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input type="text" required value={username} onChange={e => setUsername(e.target.value)}
                    className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-700 transition-all" 
              ...
              <button type="submit" disabled={isLoading} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-black tracking-wide shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2 disabled:opacity-70 mt-2">
                {isLoading ? 'VERIFYING...' : 'LOGIN TO SYSTEM'}
              </button>

              <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100">
                <button type="button" onClick={() => setView('role-select')} className="text-sm font-bold text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-1">
                  <ArrowLeft size={14} /> Back to Roles
                </button>
                <button type="button" onClick={() => { setView('forgot-username'); setError(''); }} className="text-sm font-bold text-slate-400 hover:text-indigo-600 transition-colors">
                  Forgot Password?
                </button>
              </div>
            </form>
            ...
            <form onSubmit={handleFetchQuestion} className="space-y-5 animate-in slide-in-from-right-4">
              ...
              <div className="space-y-1">
                ...
                <input type="text" required value={resetUsername} onChange={e => setResetUsername(e.target.value)}
                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-700" 
                    placeholder="Hal. encoder1" />
              </div>
              <button type="submit" disabled={isLoading} className="w-full py-4 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-black tracking-wide transition-all disabled:opacity-70">
          ...
          {view === 'forgot-question' && (
            <form onSubmit={handleResetPassword} className="space-y-5 animate-in slide-in-from-right-4">
              <div className="bg-indigo-50 p-4 rounded-xl mb-4 border border-indigo-100">
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Security Question:</span>
                <p className="font-bold text-indigo-900 mt-1">{securityQuestion}</p>
              </div>
                ...
                <div className="space-y-1 mt-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" required value={newPassword} onChange={e => setNewPassword(e.target.value)}
                      className="w-full pl-10 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-700" 
                      placeholder="••••••••" />
                  </div>
                </div>
              ...
              <button type="submit" disabled={isLoading} className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black tracking-wide shadow-lg shadow-emerald-200 transition-all mt-4 disabled:opacity-70">
      ...
      <footer className={`text-slate-400 text-[15px] font-black tracking-widest uppercase relative z-10 transition-all duration-500 ${view === 'role-select' ? 'mt-16' : 'mt-8'}`}>
