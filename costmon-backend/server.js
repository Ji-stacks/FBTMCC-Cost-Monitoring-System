...
db.serialize(() => {
  ...
  db.run("ALTER TABLE disbursements ADD COLUMN po_no TEXT", () => { });
...
app.put('/api/disbursements/:id', authenticateToken, (req, res) => {
  ...
  const s = (val) => (val === "" || val === undefined) ? null : val;
  const stmt = db.prepare('UPDATE disbursements SET project_code=?, date=?, payee=?, particulars=?, tin=?, cv_no=?, bank=?, check_no=?, or_inv_no=?, po_no=?, accts_pay=?, input_tax=?, output_tax=?, target_cib=?, gross_amount=?, ewt_amount=?, net_amount=?, expenses_json=?, costing_type=?, attachments_json=?, stocks_amount=?, stock_description=?, is_monitoring_only=? WHERE id=?');
  stmt.run(s(data.project_code), data.date, s(data.payee), s(data.particulars), s(data.tin), s(data.cv_no), s(data.bank), s(data.check_no), s(data.or_inv_no), s(data.po_no), data.accts_pay || 0, data.input_tax || 0, data.output_tax || 0, data.target_cib || 0, data.gross_amount || 0, data.ewt_amount || 0, data.net_amount || 0, JSON.stringify(data.expenses), data.costing_type || 'normal', JSON.stringify(data.attachments || []), data.stocks_amount || 0, data.stock_description || '', data.is_monitoring_only ? 1 : 0, id, function (err) {
    if (err) return res.status(500).json({ error: err.message });
